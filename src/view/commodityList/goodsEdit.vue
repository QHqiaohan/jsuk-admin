<template>
  ...
  <el-dialog :title="'商品列表' + (type == 'add'?'添加':'编辑' )" :visible="visible" @close="visible = false">
    <el-form :model="form" label-width="80px">
      <el-form-item label="商品名称">
        <el-input v-model="input" value="form.goodsName"></el-input>
        <!--<shop-select v-model="form.goodsName"/>-->
      </el-form-item>
      ...
      <el-form-item label="商品分类">
        <shop-select v-model="form.categoryInfo.name"/>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit(id) {
        this.visible = true;
        this.type = 'edit';
        this.$axios.get(`/wgoods/get`, {params: {id}})
          .then(({data: {data}}) => {
            // console.log(data);
